...
class SessionDetailViewTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = get_user_model().objects.create_user(username="testuser", email="testuser@email.com", password="testpass123")
    ...
    def test_session_detail_view(self):
        response = self.client.get(reverse("session_detail", kwargs={"pk": self.session.id}))
        self.assertEqual(response.status_code, 200)
